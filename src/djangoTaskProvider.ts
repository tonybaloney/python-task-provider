...
export class DjangoTaskProvider implements vscode.TaskProvider {
  ...
  public resolveTask(_task: vscode.Task): vscode.Task | undefined {
    ...
    var pythonPath = vscode.workspace
    ...
    if (task) {
      ...
      const pythonFile = definition.file ? definition.file : "manage.py";
      ...
      return new vscode.Task(
        ...
        new vscode.ShellExecution(
          `${pythonPath} ${pythonFile} ${definition.task} ${args}`
        )
      );
    }
    return undefined;
...
function getBuiltinTasks(): Array<string> {
  return [
    "check",
    "compilemessages",
    "createcachetable",
...
async function getDjangoTasks(): Promise<vscode.Task[]> {
  ...
  for (const workspaceFolder of workspaceFolders) {
    ...
    if (
      vscode.workspace
        .getConfiguration("pythonTaskProvider")
        .get("runDjangoToDiscoverOptions")
    ) {
      const commandLine = `${pythonPath} ${managePyFile} help`;
          ...
          for (const line of lines) {
            ...
            if (isCommandList) {
              ...
              const task = new vscode.Task(
                ...
                taskName,
                "django",
                new vscode.ShellExecution(`${pythonPath} ${managePyFile} ${taskName}`)
              );
              result.push(task);
      ...
      getBuiltinTasks().forEach((taskName) => {
        const kind: DjangoTaskDefinition = {
          type: "django",
          task: taskName,
        };
