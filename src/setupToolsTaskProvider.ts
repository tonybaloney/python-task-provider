  ...
  for (const workspaceFolder of workspaceFolders) {
    ...
    if (
      vscode.workspace
        ...
        .get("runSetupToolsToDiscoverOptions")
    ) {
      const commandLine = `${pythonPath} ${setupPyFile} --help-commands`;
          ...
          for (const line of lines) {
            ...
            if (isStandardCommands || isExtraCommands) {
              ...
              const task = new vscode.Task(
                ...
                taskName,
                "setuptools",
                new vscode.ShellExecution(`${pythonPath} setup.py ${taskName}`)
              );
              result.push(task);
      ...
      getBuiltinTasks().forEach((taskName) => {
        ...
        const task = new vscode.Task(
          ...
          taskName,
          "setuptools",
          new vscode.ShellExecution(`${pythonPath} ${setupPyFile} ${taskName}`)
        );
        result.push(task);
